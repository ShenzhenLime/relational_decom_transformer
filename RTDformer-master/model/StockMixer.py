...
class Model(nn.Module):
    def __init__(self,configs, stocks=155, market=1, scale=3):
        '''
        stocks=1026
        time_steps=16
        channels=5
        ...
        stocks=configs.num_stock
        time_steps=configs.seq_len
        channels=configs.enc_in
        scale_dim = 48
        self.mixer = MultTime2dMixer(time_steps, channels, scale_dim=scale_dim)
        self.channel_fc = nn.Linear(channels, 1)
        self.time_fc = nn.Linear(time_steps * 2 + scale_dim, 48)
        ...
        self.time_fc_ = nn.Linear(time_steps * 2 + scale_dim, 48)
        
        self.expand_fc = nn.Linear(1, 2) 
...
#         print("测试：",y.shape)  #[77,240,4]
        ...
        combined=self.expand_fc(combined)
        

        
#         print(combined.shape)
        output = F.elu((combined))
